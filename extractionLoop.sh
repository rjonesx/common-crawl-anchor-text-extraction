...
hadoop fs -copyToLocal s3n://anchorcc/anchor-extractor-1.1.jar ~/anchor-extractor-1.1.jar
...
while [ "$lastSegment" != "" ]; do 
  ...
  then
   ...
   break
	...
	if [ "$modeval" = "anchor" ]; 
     then 
       hadoop jar ~/anchor-extractor-1.1.jar com.moz.commoncrawl.WATAnchorExtractor -D anchors.track.hosts=true "$lastSegment" s3n://anchorcc/anchors-hosts/
     else 
       hadoop jar ~/anchor-extractor-1.1.jar com.moz.commoncrawl.WATLinkExtractor "$lastSegment" s3n://anchorcc/links/
	fi
